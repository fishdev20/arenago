...
const Map = forwardRef<MapRef, MapProps>(function Map({ children, styles, ...props }, ref) {
  ...
  useEffect(() => {
    ...
    // ✅ snapshot camera before style swap
    const camera = {
    ...
    const restoreCamera = () => {
      // ✅ restore camera after new style loads
      mapInstance.jumpTo({
      ...
      // Helps after style swap / view transitions
      mapInstance.resize()

      mapInstance.off('style.load', restoreCamera)
    }

    mapInstance.on('style.load', restoreCamera)
